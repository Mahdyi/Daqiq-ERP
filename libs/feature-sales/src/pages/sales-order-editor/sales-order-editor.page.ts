...
import { formatNumber } from '../../config/sales-order-table.config';
import {
...
const DEFAULT_LINE_DRAFT: SalesOrderLineFormValue = {
  clientId: '',
  productId: null,
  description: null,
  quantity: 1,
...
@Component({
  selector: 'daqiq-sales-order-editor-page',
  imports: [
    PageContainerComponent,
    CardComponent,
    DynamicFormComponent,
  ...
  templateUrl: './sales-order-editor.page.html',
  styleUrl: './sales-order-editor.page.scss',
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class SalesOrderEditorPage implements OnInit {
  protected readonly facade = inject(SalesOrderEditorFacade);
  ...
  protected readonly title = this.isEditMode ? 'ویرایش سفارش فروش' : 'ایجاد سفارش فروش';
  protected readonly lineDraft = signal<SalesOrderLineFormValue>(DEFAULT_LINE_DRAFT);
  protected readonly lineError = signal<string | null>(null);
  protected readonly headerInitialValue = computed(() =>
    this.isEditMode ? this.facade.initialValue() : DEFAULT_SALES_ORDER_FORM_VALUE
  );
  ...
  protected async handleSubmit(event: FormSubmitEvent<SalesOrderFormValue>): Promise<void> {
    ...
    if (order) {
      await this.router.navigate(['/sales/sales-orders', order.id]);
    }
  }
